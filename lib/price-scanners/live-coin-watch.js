...
export default class LiveCoinWatchPriceScanner extends BasePriceScanner {
	...
	async _getPrice(code) {
		if (!this.coinIdByCode.has(code)) return undefined
	...
	async getAndCacheAllPrices() {
		...
		const priceByCode = await this.getPrices([...this.coinIdByCode.values()])
	 ...
	 * @returns {Promise<Map<enums.AssetCode, number>>}
	 */
	async getPrices(coinIds = []) {
		...
		/** @type {Map<enums.AssetCode, number>} */
		const priceByCode = new Map()
		for (const [code, coinId] of this.coinIdByCode) {
