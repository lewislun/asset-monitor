	...
	static async storeScanResult(scanResult = [], opts = {}) {
		...
		return await startOrInheritTransaction(async (trx) => {
			...
			await Promise.all(scanResult.queryResults.map(result => new Promise(async resolve => {
				...
				if (result.groupSpecifier === undefined) {
					logger.debug(`Query missing groupSpecifier, using default group... - accountId: ${result.accountId}, defaultGroupName: ${constants.DEFAULT_ASSET_GROUP_NAME}`)
					result.groupSpecifier = await BaseModel.AssetGroup.getGroupId(constants.DEFAULT_ASSET_GROUP_NAME)
				}
				resolve()
			})))
