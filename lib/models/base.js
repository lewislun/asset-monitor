 ...
 */
export async function refreshMaterializedViews(knex = undefined) {
	if (!knex) knex = Model.knex()
	await knex.schema.refreshMaterializedView(BaseModel.BatchListView.tableName)
	await knex.schema.refreshMaterializedView(BaseModel.SummaryView.tableName)
}