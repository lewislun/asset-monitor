...
export default class BaseCosmosAssetScanner extends BaseAssetScanner {
	...
	async _init() {
		await super._init()
		this.tmClient = await Tendermint34Client.connect(this.params.endpoint)
		this.client = await StargateClient.connect(this.params.endpoint)
		this.cwClient = await CosmWasmClient.connect(this.params.endpoint)
	}
