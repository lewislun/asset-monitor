...
export default class CosmosSecondaryTokenScanner extends BaseCosmosAssetScanner {
	...
	async _query(assetQuery) {
		...
		for (const [code, contractAddr] of this.assetInfoMap) {
			const contractQuery = { balance: { address: assetQuery.addr } }
			const [ balanceStr, price, tokenInfo, block ] = await Promise.all([
			...
			const balance = parseDecimal(balanceStr, tokenInfo.decimals)
			results.push({
				name: tokenInfo.name,
				code,
				chain: this.chain,
