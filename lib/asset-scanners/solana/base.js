...
export default class BaseSolanaAssetScanner extends BaseAssetScanner {
	...
	/** @type {Map<solanaWeb3.PublicKey, number>} */	static decimalsByMint = new Map()
	 ...
	 * @returns {Promise<number>}
	 */
	async getSplTokenDecimals(mint) {
		const cachedValue = BaseSolanaAssetScanner.decimalsByMint.get(mint)
		if (cachedValue !== undefined) return cachedValue

		const result = await this.rateLimiter.exec(async () => await this.connection.getTokenSupply(mint))
		BaseSolanaAssetScanner.decimalsByMint.set(mint, result.value.decimals)
