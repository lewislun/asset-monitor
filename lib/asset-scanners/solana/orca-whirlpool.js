...
import { int2Decimal } from '../../utils.js'
...
export default class OrcaWhirlpoolScanner extends BaseSolanaAssetScanner {
	...
	async _query(assetQuery) {
		...
		const { context, value: accResults } = await this.rateLimiter.exec(async () => await this.connection.getParsedTokenAccountsByOwner(pubkey, { programId: TOKEN_PROGRAM_ID }))
		...
		for (let accResult of accResults) {
			...
			const positionData = await this.rateLimiter.exec(async () => await this.accFetcher.getPosition(pda.publicKey, true))
			if (!positionData) continue
			const position = await this.rateLimiter.exec(async () => await this.client.getPosition(pda.publicKey, true))

			// get whirlpool and token data
			const pool = await this.rateLimiter.exec(async () => await this.client.getPool(positionData.whirlpool, true))
			...
			const tokenPrices = []
			for (let i = 0; i < 2; i++) {
				tokenAmounts.push(int2Decimal(tokenAmountU64s[i], tokenInfos[i].decimals))
				tokenCodes.push(ASSET_CODE_BY_ADDR[tokenInfos[i]?.mint.toBase58()])
			...
			for (const i in tokenAmountU64s) {
				queryResults.push({
					name: assetName,
					code: tokenCodes[i],
					chain: OrcaWhirlpoolScanner.chain,
					type: enums.AssetType.LIQUIDITY,
			...
			for (const i in rewardsQuote) {
				...
				// TODO: get decimal cache
				const tokenSupply = await this.rateLimiter.exec(async () => await this.connection.getTokenSupply(tokenInfo.mint))
				const rewardAmount = int2Decimal(rewardAmountBN, tokenSupply.value.decimals)
				const code = ASSET_CODE_BY_ADDR[tokenInfo.mint.toBase58()]
			...
			for (const i in feeAmountBNs) {
				...
				const feeAmount = int2Decimal(feeAmountBN, tokenInfos[i].decimals)
