...
export default class BaseAssetScanner extends BaseService {
	...
	async query(query) {
		...
		let snapshots = await this._query(query)
		...
		snapshots = await Promise.all(snapshots.map(async snapshot => {
			...
			return {
				...
				// inject extra tags
				tag_map: { ...(query.extra_tag_map ?? {}), ...(snapshot.tagMap ?? {}) },
				// patch chain
				chain: snapshot.chain ?? this.chain,
				// patch account_id
				account_id: snapshot.account_id ?? query.addr ?? query.apiKey ?? apiSecret,
				// patch group_id
				group_id: snapshot.group_id ?? query.group_id ?? await AssetGroup.getDefault()?.id,
				// patch query_id
				query_id: snapshot.query_id ?? query.id,
			}
		}))
