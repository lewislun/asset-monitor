	...
	async query(assetQuery) {
		...
		if (!Array.isArray(this.constructor.requiredQueryKeys)) {
			...
			this.constructor.requiredQueryKeys.forEach(key => {
				if (assetQuery[key] === undefined || assetQuery[key] === null) throw new Error(`Missing query field: ${key} - scannerClass: ${this.constructor.name}`)
			})
		}

		// Do query
		const stringifiedQuery = JSON.stringify(assetQuery, undefined, 2)
		const startTime = new Date().getTime()
		logger.debug(`Start querying asset - scanner: ${this.constructor.name}, query: ${stringifiedQuery}`)
		const checkStuckLogInterval = setInterval(() => {
			logger.info(`Still querying asset - scanner: ${this.constructor.name}, timeUsed: ${(new Date).getTime() - startTime}ms, query: ${stringifiedQuery}`)
		}, 10000)
		...
		results.forEach(result => {
			// inject extra tags
			if (assetQuery.extra_tag_map) result.tagMap = { ...assetQuery.extra_tag_map, ...(result.tagMap ?? {}) }
			...
			if (!result.accountId) {
				...
				// mask apiSecret
				if (result.accountId === assetQuery.api_secret) {
					result.accountId = result.accountId.substring(0, 3) + '*'.repeat(result.accountId.length - 3)
				}
			}

			// patch groupSpecifier
			if (!result.groupSpecifier) result.groupSpecifier = assetQuery.group_id ?? constants.DEFAULT_ASSET_GROUP_NAME
		})

		clearInterval(checkStuckLogInterval)
		logger.debug(`Finished querying asset - scanner: ${this.constructor.name}, timeUsed: ${(new Date).getTime() - startTime}ms, query: ${stringifiedQuery}`)
		return results
	}
