...
import { AssetFlow, AssetGroup, SummaryView, AssetSnapshotBatch } from './models/index.js'
	...
	async initAssetMonitorCommands(assetMonitor) {
		await this.addCommand({
			...
			handler: async (ctx) => {
				...
				await AssetFlow.recordFlow(fromGroup?.id, toGroup?.id, amountStr)
				await ctx.reply(`Asset flow recorded\\.`)
			}
		})
