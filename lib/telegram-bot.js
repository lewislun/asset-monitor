...
export default class AssetMonitorTelegramBot extends TelegramCommander {
	...
	async initAssetMonitorCommands(assetMonitor) {
		...
		await this.addCommand({
			...
			handler: async (ctx) => {
				const assetPercentages = await analytics.getAssetPercentages({ batchId: latestBatch.id })
				const rows = assetPercentages
					.sort((a, b) => b.usdValue.cmp(a.usdValue))
					.map(row => {
						return e(`${row.chain} ${row.code} ${row.type} ${row.state}: ${row.usdValue.toDP(2)} USD (${row.percentage.toDP(2)}%)`)
					})
