...
export default class AssetMonitor {
	...
	async recordFlow(fromGroupSpecifier, toGroupSpecifier, value, opts = {}) {
		return await startOrInheritTransaction(async (trx) => {
			...
			// get pnl% of fromGroup
			if (fromGroupId) {
				logger.info(`Getting pnl% of group ${fromGroupId} to calculate invested value...`)
				const scanRes = await this.scan({ groupIds: [fromGroupId] })
				const actualValue = scanRes.totalUSDValue
				const pastInvestedValue = await AssetFlow.getTotalInvestedValue({ trx, groupIds: [fromGroupId] })
				const pnlRatio = pastInvestedValue.eq(0) ? new Decimal(1) : actualValue.div(pastInvestedValue)
				investedValue = investedValue.mul(pnlRatio)
				logger.info(`group ${fromGroupId} - pastInvestedValue: ${pastInvestedValue}, pnlRatio: ${pnlRatio.sub(1).mul(100)}%`)
			}
			...
			const flow = await AssetFlow.query(trx).insert({
				...
				actual_value: new Decimal(value).toString(),
