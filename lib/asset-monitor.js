...
export default class AssetMonitor {
	...
	constructor(opts = {}) {
		...
		// load secrets
		logger.info(`Loading screts: ${path.resolve(secretsPath)}`)
		...
		/** @type {MonitorConfig} */
		const monitorConfig = this.loadConfig(monitorConfigPath, secretObj)
	 ...
	 * @returns {ScannersConfig | MonitorConfig}
	 */
	...
	/**
	 * @public
	 * @returns {Promise<{ queryResults: types.AssetQueryResult[], totalUSDValue: Decimal }>}
	 */
	async scan() {
		...
		const totalUSDValue = queryResults.reduce((acc, cur) => acc.add(cur.usdValue), new Decimal(0))
		logger.info(`Finished scanning assets - totalUSDValue: ${totalUSDValue}, resultCount: ${queryResults.length}`)
		return { queryResults, totalUSDValue }
	}
	 ...
	 */
	async monitor() {
		logger.info(`Start monitoring assets with cron: '${this.cron}'...`)
		cron.schedule(this.cron, async () => {
			const { queryResults } = await this.scan()
			logger.info('Storing query results...')
			const batch = await AssetSnapshotBatch.storeResults(queryResults)
	...
	addPriceScanner(config) {
		...
		logger.info(`PriceScanner added - name: ${scannerClass.constructor.name}, config: ${JSON.stringify(config)}`)
	}
	...
	setupMonitor(config) {
		...
		this.cron = config.cron ?? this.cron
	}
