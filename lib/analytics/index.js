...
export async function getTotalValue(opts = {}) {
	...
	const totalValue = await latestBatch.$relatedQuery('snapshots').sum('usd_value as totalValue').first()
	return new Decimal(totalValue?.totalValue ?? 0)
}
 ...
 * @returns {Promise<analyticsTypes.TotalValueData[]>}
 */
export async function getAssetPercentages(opts = {}) {
	// get total value
	const totalValue = await getTotalValue(opts)
	// get latest batch number
	const latestBatch = await AssetSnapshotBatch.query(opts?.trx).orderBy('scan_started_at', 'desc').first()
	// get asset percentages
	const query = AssetSnapshot.query(opts?.trx)
		.select('chain', 'code', 'type', 'state')
		.sum('usd_value as totalValue')
		.groupBy('chain', 'code', 'type', 'state')
		.where('batch_id', latestBatch.id)
	const rows = await query
	return rows.map(row => {
		const value = new Decimal(row.totalValue)
		/** @type {analyticsTypes.TotalValueData} */
		const data = {
			chain: row.chain,
			code: row.code,
			type: row.type,
			state: row.state,
			usdValue: value,
			percentage: value.div(totalValue).mul(100),
		}
		return data
	})
}
