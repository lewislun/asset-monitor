		...
		switch (opts.groupBy) {
			...
			case 'state':
				...
				distinctQuery = trx.from(trx(AssetSnapshot.tableName).distinct(joinField).as('s'))
					.crossJoin(trx(AssetSnapshotBatch.tableName).select('id', 'scan_finished_at').as('b'))
			...
			case undefined:
				...
				distinctQuery = trx(AssetSnapshotBatch.tableName).select(`id as ${joinField}`, 'scan_finished_at as time')
				break
			default:
